...
import { useTabSectionQuery } from '../../hooks';
...
const TabSection = ({ headingHash }) => {
  const intl = useIntl();
  const { getImportTabReact, getImportTabNg, getDemoTabReact, getDemoTabNg } = useTabSectionQuery();
  ...
  const getReactVersionContent = () => {
    ...
    return (
      <TabContentWrapper>
        ...
        <ComponentDemoBox demo={getDemoTab()} codeInHtml={getDemoTabReact.nodes[0].html} />

        <MarginalHeading3 css={marginTopForHeading3}>
          {intl.formatMessage({ id: 'components.shared.imports' })}
        </MarginalHeading3>
        <CodeViewer codeInHtml={getImportTabReact.nodes[0].html} />
  ...
  const getAngularVersionContent = () => {
    ...
    return (
      <TabContentWrapper>
        ...
        <Subtitle2>Default:</Subtitle2>
        <ComponentDemoBox demo={getDemoTab()} codeInHtml={getDemoTabNg.nodes[0].html} />

        <MarginalHeading3 css={marginTopForHeading3}>
          {intl.formatMessage({ id: 'components.shared.imports' })}
        </MarginalHeading3>
        <CodeViewer codeInHtml={getImportTabNg.nodes[0].html} />
